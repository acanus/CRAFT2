...
def get_result_img(image, score_text, score_link, text_threshold = 0.68, link_threshold = 0.4, low_text = 0.08, ratio_w = 1.0, ratio_h = 1.0, dirname = 'text_image'):
    boxes = getDetBoxes(score_text, score_link, text_threshold, link_threshold, low_text, s = False)
    boxes = adjustResultCoordinates(boxes, ratio_w, ratio_h)
    saveResult(os.path.join(dirname, 'text_image_char.jpg'), image, boxes, dirname = dirname)
    boxes = getDetBoxes(score_text, score_link, text_threshold, link_threshold, low_text, s = True)
    boxes = adjustResultCoordinates(boxes, ratio_w, ratio_h)
    saveResult(os.path.join(dirname, 'text_image_word.jpg'), image, boxes, dirname = dirname)