...
from affinity_util import cal_affinity_boxes, reorder_points
from pseudo_util import crop_image, watershed, find_char_box, un_warping, conf, divide_region, enlarge_char_boxes
    ...
    for word_box, word_length in zip(word_boxes, word_lengths):
        if word_length > 0:
            try:
                ...
                confidence_value = conf(region_boxes, word_length)
...
class Fake(tf.keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        def tf_create_pseudo_gt(input_box, input_word_length, input_region, input_affinity, input_confidence, region_pred):
